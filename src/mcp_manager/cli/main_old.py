...
def _prompt_for_server_configuration(server_name: str, server_type: ServerType, package: Optional[str], test_mode: bool = False) -> Optional[dict]:
...
def nuke(force: bool):
    ...
    console.print("💥💥💥 [red bold]NUCLEAR OPTION: Clearing ALL MCP configurations[/red bold] 💥💥💥")
    ...
    # 1. Kill zombie processes first
    console.print("\n🔪 [yellow]Killing zombie MCP processes...[/yellow]")
    killed_processes = 0
    
    try:
        import subprocess
        # Kill any hanging claude mcp processes
        result = subprocess.run(["pkill", "-f", "claude.*mcp"], capture_output=True, timeout=5)
        if result.returncode == 0:
            killed_processes += 1
            console.print("  ✅ Killed hanging claude mcp processes")
        
        # Kill any MCP server processes
        result = subprocess.run(["pkill", "-f", "mcp.*server"], capture_output=True, timeout=5)
        if result.returncode == 0:
            killed_processes += 1
            console.print("  ✅ Killed MCP server processes")
            
    except Exception as e:
        console.print(f"  ⚠️ Could not kill processes: {e}")
    
    # 2. Clean up Docker containers and configurations
    console.print("\n🐳 [blue]Cleaning up Docker MCP containers and configs...[/blue]")
    docker_cleaned = 0
    
    try:
        # Stop and remove ALL containers (not just MCP labeled ones)
        ...
        if result.returncode == 0 and result.stdout.strip():
            ...
            for container_id in container_ids:
                if container_id:
                    # Get container info to see if it's MCP related
                    info_result = subprocess.run(["docker", "inspect", container_id], 
                                                capture_output=True, timeout=5)
                    if info_result.returncode == 0:
                        info_text = info_result.stdout.decode()
                        if any(term in info_text.lower() for term in ['mcp', 'filesystem', 'sqlite', 'claude']):
                            ...
                            docker_cleaned += 1
                            console.print(f"  ✅ Removed MCP-related container: {container_id[:12]}")
        
        # Also disable Docker Desktop MCP servers
        console.print("  🛑 Disabling Docker Desktop MCP servers...")
        for server_name in ['filesystem', 'sqlite', 'search', 'http', 'k8s', 'terraform', 'aws', 'SQLite', 'aws-diagram', 'github']:
            try:
                result = subprocess.run(["docker", "mcp", "server", "disable", server_name], 
                                      capture_output=True, timeout=10)
                if result.returncode == 0:
                    console.print(f"  ✅ Disabled Docker Desktop server: {server_name}")
            except:
                pass  # Ignore errors for servers that don't exist
                
        # Clean up any docker-compose files in the project
        compose_files = [
            Path.cwd() / "docker-compose.yml",
            Path.cwd() / "docker-compose.yaml", 
            Path.home() / ".docker" / "compose" / "mcp-compose.yml"
        ]
        
        for compose_file in compose_files:
            if compose_file.exists():
                compose_file.unlink()
                console.print(f"  ✅ Removed: {compose_file}")
                docker_cleaned += 1
                
        if docker_cleaned == 0:
            console.print("  ℹ️ No MCP Docker containers or configs found")
            
    except Exception as e:
        console.print(f"  ⚠️ Could not clean Docker containers: {e}")
    
    # 3. Clear config files
    console.print("\n📁 [cyan]Clearing configuration files...[/cyan]")
    config_files = [
        Path.home() / ".config" / "claude-code" / "mcp-servers.json",
        Path.cwd() / ".mcp.json",
        Path.home() / ".config" / "mcp-manager" / "mcp_manager.db",
        Path.home() / ".config" / "mcp-manager" / "workflows.json",
        # Docker Desktop MCP config
        Path.home() / ".docker" / "cli-plugins" / "docker-mcp" / "config.yaml",
        Path.home() / ".docker" / "mcp-config.yaml"
    ]
        ...
        try:
            if config_file.exists():
                if config_file.name.endswith('.json'):
                    # Clear JSON config files
                    if 'mcp-servers' in config_file.name:
                        config_file.write_text('{"mcpServers": {}}')
                    else:
                        config_file.write_text('{}')
                    console.print(f"  ✅ Cleared: [cyan]{config_file}[/cyan]")
                    cleared_count += 1
                elif config_file.name.endswith('.db'):
                    # Remove database files
                    config_file.unlink()
                    console.print(f"  ✅ Removed: [cyan]{config_file}[/cyan]")
                    cleared_count += 1
        except Exception as e:
            console.print(f"  ⚠️ Could not clear [cyan]{config_file}[/cyan]: {e}")
    
    # Summary
    console.print(f"\n💥💥💥 [green]Nuclear reset complete![/green] 💥💥💥")
    console.print(f"  🔪 Killed: {killed_processes} zombie processes")
...
def status():
    ...
    async def show_status():
        try:
            manager = cli_context.get_manager()
            console.print("[bold blue]📊 MCP Manager System Status[/bold blue]\n")
            
            # 1. Server Summary
            servers = manager.list_servers_fast()
            active_servers = [s for s in servers if s.enabled]
            inactive_servers = [s for s in servers if not s.enabled]
            
            from rich.panel import Panel
            
            # Server status panel
            server_status = f"""[green]Active Servers:[/green] {len(active_servers)}
[yellow]Inactive Servers:[/yellow] {len(inactive_servers)}
[cyan]Total Servers:[/cyan] {len(servers)}"""
            
            console.print(Panel(server_status, title="[bold green]🖥️ Server Status[/bold green]", border_style="green"))
            ...
            try:
                ...
                tools_status = f"""[green]Total Tools:[/green] {stats.get('total_tools', 0)}
[green]Available Tools:[/green] {stats.get('available_tools', 0)}
[cyan]Servers with Tools:[/cyan] {stats.get('servers_with_tools', 0)}"""
                
                console.print(Panel(tools_status, title="[bold blue]🔧 Tools Registry[/bold blue]", border_style="blue"))
            except Exception as e:
                console.print(Panel(f"[red]Tools registry unavailable: {e}[/red]", title="[bold red]🔧 Tools Registry[/bold red]", border_style="red"))
            ...
            try:
                ...
                workflow_status = f"""[green]Total Workflows:[/green] {len(all_workflows)}
[green]Active Workflow:[/green] {active_workflow.name if active_workflow else 'None'}"""
                
                if active_workflow:
                    workflow_status += f"\n[cyan]Active Suites:[/cyan] {len(active_workflow.suite_ids)}"
                
                console.print(Panel(workflow_status, title="[bold magenta]⚡ Workflow Status[/bold magenta]", border_style="magenta"))
            except Exception as e:
                console.print(Panel(f"[red]Workflow system unavailable: {e}[/red]", title="[bold red]⚡ Workflow Status[/bold red]", border_style="red"))
            ...
            try:
                ...
                analytics_status = f"""[green]Usage Entries (7d):[/green] {summary.get('total_usage_entries', 0)}
[green]Tool Calls (7d):[/green] {summary.get('total_tool_calls', 0)}
[cyan]Success Rate:[/cyan] {summary.get('overall_success_rate', 0.0):.1%}"""
                
                console.print(Panel(analytics_status, title="[bold yellow]📈 Analytics (7 days)[/bold yellow]", border_style="yellow"))
            except Exception as e:
                console.print(Panel(f"[red]Analytics unavailable: {e}[/red]", title="[bold red]📈 Analytics[/bold red]", border_style="red"))
            
            # 5. System Health
            system_info = manager.get_system_info()
            health_issues = []
            
            # Handle both dict and SystemInfo object formats
            claude_available = system_info.get('claude_available', False) if isinstance(system_info, dict) else getattr(system_info, 'claude_cli_available', False)
            
            if not claude_available:
                health_issues.append("Claude CLI not available")
            
            if health_issues:
                health_status = "[red]Issues Found:[/red]\n" + "\n".join(f"• {issue}" for issue in health_issues)
                console.print(Panel(health_status, title="[bold red]⚠️ System Health[/bold red]", border_style="red"))
            else:
                health_status = "[green]✅ All systems operational[/green]"
                console.print(Panel(health_status, title="[bold green]✅ System Health[/bold green]", border_style="green"))
            
            # 6. Quick Configuration Info
            from mcp_manager.utils.config import get_config
            import platform
            import sys
            
            config = get_config()
            config_status = f"""[cyan]Config Directory:[/cyan] {config.config_dir}
[cyan]Platform:[/cyan] {platform.system()}
[cyan]Python:[/cyan] {sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro}"""
            
            log_file = config.get_log_file()
            if log_file:
                config_status += f"\n[cyan]Log File:[/cyan] {log_file}"
            
            console.print(Panel(config_status, title="[bold dim]⚙️ Configuration[/bold dim]", border_style="dim"))
